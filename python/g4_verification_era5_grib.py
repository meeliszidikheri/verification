...
""" usage: python3 g4_verification_era5_grib.py suite_name cycle field pressure_level timestep domain spec_range type ver_option outpath nens
           ver_res
    ...
    type: mean, stdev, var, prob? (where ? is a threshold value e.g., 'prob4000' would be interpreted as the probability
                                   of exceeding a value of 4000 if field is 'geopotential_height'; note that this doesn't
...
def get_ens_field(suite_name, cycle, field, field_name, field_name_long, pressure_level, surf, timestep):
    ...
    timestep_lag_18h = str(int(timestep) + 18)
    ...
    if 'accumulated' in field:
        ...
        base_timestep_lag_18h = int(timestep_lag_18h) - accum_time
    # for rainfall rate, accumulated rainfall over 6 h needs to be computed 
        ...
        base_timestep_lag_18h = timestep
    ...
    for infile in infiles:
        ...
        elif ens_nums[cnt] >= 36 and ens_nums[cnt] < 54:
            timestep = timestep_lag_12h
            base_timestep = base_timestep_lag_12h
        else:
            timestep = timestep_lag_18h
            base_timestep = base_timestep_lag_18h
...
def get_data_path(suite_name, cycle, timestep, field_name, surf):
    ...
    # compute lagged cycle (-18 h):
    ta = datetime.datetime.strptime(cycle, "%Y%m%dT%H%MZ")
    tlag = ta - datetime.timedelta(18.0/24.0)
    cycle_lag_18h = tlag.strftime("%Y%m%dT%H%MZ")
    ...
    for iens in range(first, last):
        ...
        # path to data file
            dir = "/scratch/dp9/mjz548/marsdata/" + suite_name + "/" + cycle_lag_18h \
                + "/" + str(iens-54)
            if field_name == 'unknown':
                flnm = "data_precip.grib"
            elif surf == True:
                flnm = "data_sfc_lag18.grib"
            else:
                flnm = "data_lag18.grib"
...
def main():
    ...
    if ver != '0':
        ...
        if suite_name == 'g3':
            ...
            # read min, max values from g3 file
